...
* **Stepped UI & Dashboards:** Utilize standard shadcn components (Tabs, Progress, etc.) augmented with brand colors.
...
## 6. Execution & Automation Workflow
This design system is designed to be implemented autonomously using agent skills.
- **Stitch Loop (`stitch-loop`):** Use this skill to iteratively build the website, generating pages via Stitch and passing the baton between tasks. Read `.stitch/next-prompt.md` to pick up tasks.
- **React Components (`react-components`):** Use this skill to convert the generated Stitch designs into modular Vite + React components, specifically replacing standard HTML with the mandated **shadcn/ui** and **Tailwind CSS** equivalents as detailed in Section 4.
